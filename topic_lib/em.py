...
def estep(gamma, x, log_p, log_theta):
    ...
    for w, d in zip(nz_words,nz_docs):
        gamma[w,d,:] = x[w,d] * (log_p[:,d] * log_theta[w,:]) / (np.sum(log_p[:,d] * log_theta[w,:]))
...
def mstep(x, gamma, pseudo_count=0.01):
    ...
    for d in range(num_docs):
        log_p[:,d] = np.sum(gamma, axis = 0)[d] / (np.sum(np.sum(gamma, axis=0), axis = 1))[d]

    #print(np.transpose(np.sum(gamma, axis = 1))[0])
    #print(np.sum(np.sum(gamma, axis=1),axis=1))

    for t in range(num_topics):
        log_theta[:,t] = np.transpose(np.sum(gamma, axis = 1))[t] / (np.sum(np.sum(gamma, axis=1), axis = 1))
...
def get_loglik(x, log_p, log_theta):
    ...
    for w, d in zip(nz_words, nz_docs):
        ...
        res += x[w,d] * np.log(log_p[:,d]*log_theta[1,:])
